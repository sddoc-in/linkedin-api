...
@app.get("/open-google")
async def open_google(driver_session: tuple =Depends(get_driver)):
    try:
...
@app.get("/search")
async def search_google(query: str = Query(...), session_id: str = Query(...), driver=Depends(get_session_driver)):
    try:
